...
- The typed known-failure ledger is
  ...
  Those cases run and must fail with their declared protocol status; they are
  not skipped.

## Build and verify

```sh
cmake -S c -B c/build -DCAPNWEB_SANITIZE=ON
cmake --build c/build --parallel
ctest --test-dir c/build --output-on-failure
npx vitest run __tests__/c-interop.test.ts
```

`capnweb-resource-profile` reports table/scratch sizes and elapsed host CPU
time. `libraryHeapPolicy: "no allocator dependency"` describes the library
contract; it is intentionally not a fabricated count of allocations made by
the host C runtime.
